# 更新日志 (CHANGELOG)

所有 noteworthy 的变更都将记录在此文件中。

## [V2.4.0-beta2.1] - 2025-10-07

### 新增功能
- **多加密方式支持**: 添加 ChaCha20-Poly1305 加密算法，支持流式加密解密
- **设置选项**: 添加加密方式选择、版本号显示开关和复制选项
- **加密方式选择**: 用户可在 AES-128、AES-192、AES-256 和 ChaCha20-Poly1305 之间切换
- **版本显示控制**: 用户可选择是否在界面中显示版本号
- **复制功能增强**: 用户可选择复制时使用解密文件或加密文件

### 变更
- 重构加密解密逻辑以支持多种算法
- 保留对原有加密文件的向后兼容性

## [V2.3.0-beta5] - 2025-09-28

### 新增功能
- **右键菜单删除文件**: 在加密文件列表中，现在可以通过右键单击文件并选择"删除文件"选项来删除指定的加密文件及其关联的元数据文件。

### 修复
- 修复了 `main.py` 文件中因添加右键菜单功能时引入的语法错误，该错误导致程序无法运行。
- 优化了 `delete_selected_file` 方法中的结果显示信息，使其更清晰地反映删除的文件。

### 其他
- 调整了 `FileEncryptionApp` 类中的 UI 代码，添加了右键事件绑定 (`bind("<Button-3>", ...)`) 以支持新功能。

## [V2.3.0-beta5] - 2025-09-28

### 新增功能
- **右键菜单删除文件**: 在加密文件列表中，现在可以通过右键单击文件并选择“删除文件”选项来删除指定的加密文件及其关联的元数据文件。

### 修复
- 修复了 `main.py` 文件中因添加右键菜单功能时引入的语法错误，该错误导致程序无法运行。
- 优化了 `delete_selected_file` 方法中的结果显示信息，使其更清晰地反映删除的文件。

### 其他
- 调整了 `FileEncryptionApp` 类中的 UI 代码，添加了右键事件绑定 (`bind("<Button-3>", ...)`) 以支持新功能。